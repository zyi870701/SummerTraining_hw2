...
model.add(Convolution2D(32, 3, 3, border_mode='same', name='conv1_1'))
...
cnn_model = model_from_json(open(model_path).read())
cnn_model.load_weights(weights_path)

for k in range(1, len(model.layers)):
	weights = cnn_model.layers[k-1].get_weights()
...
kept_filters = []
for filter_index in range(0, scan_filter_num):
